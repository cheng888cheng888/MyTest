...
public class ZhixingrenTestRule3 implements RxRule{
	@Override
	public RxResult execute(RxTask myTask, RxCrawler myCrawler, RxDatabase myDatabase) throws RxCrawlerException {
		...
		try
		{
			...
			//获取公司编码
			String bzxr_code = myTask.getV4();
			if(bzxr_code.equals("") || "-1".equals(bzxr_code))
			{
			...
			int finish_code = exe.doExecute(url, bzxr_name, bzxr_code, resource_code, version);
	 ...
	 * @throws Exception
	 */
	private void save(RxDatabase myDatabase, List<Map<String,String>> lstEntity, String version, String bzxr_code) throws Exception {
	...
	public class ZhingxingrenInfoExecutor {
		...
		public int doExecute(String url,String bzxr_name, String bzxr_code, String resource_code, String version) throws Exception {
			 
			myCrawler.open(url, "全国法院被执行人信息查询 - 被执行人查询");
			sleep(5000);
			int finish_code = capture(bzxr_name, bzxr_code, resource_code, version);
			return finish_code;
		}

		private int capture(String bzxr_name, String bzxr_code, String resource_code, String version) {
		...
		try {
				...
				if(page_info != null && page_info.length > 0)
				{
					...
					{
						List<Map<String,String>> ZhixingrenInfoListTemp = GetZhixingrenInfo(resFieldInfoList);
					...
					{
						//保存数据
						save(myDatabase, ZhixingrenInfoList, version, bzxr_code);
		 ...
		 * 分页获取执行人信息
		 */
		private List<Map<String,String>> GetZhixingrenInfo(List<ResourceFieldInfo> resFieldInfoList)
		{
			//myTask.log("3");
			List<Map<String,String>> ZhixingrenInfoList = new ArrayList<Map<String,String>>();
			...
			if(nodeSearch != null)
			{
				...
				{		
					...
					for(int k = 1;k<= lstTr.size();k++)
					{
						...
						RxNode node;
						String nodeText = "";
						Map<String,String> zxrInfo = new HashMap<String,String>();
						for(ResourceFieldInfo item : resFieldInfoList)
						{
							...
							zxrInfo.put(item.getResource_field_name(), nodeText);
							/*
	 ...
	 * 执行人信息
	 */
	public static class ZhixingrenInfo2 {
