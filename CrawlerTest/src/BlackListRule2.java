...
public class BlackListRule2 implements RxRule {
	@Override
	public void execute(RxTask myTask, RxCrawler myCrawler, RxDatabase myDatabase,
		...
		List<CompanyBlackListInfo> lstEntity = exe.doExecute();
		String result = "";
		
		if(lstEntity != null && lstEntity.size() > 0)
		{
			for(CompanyBlackListInfo item : lstEntity)
			{
				result += item.getCompany_name() + item.getOrganization_id() + item.getBlacklist_type() + item.getArea() + "\r\n";
			}
		...
		private List<CompanyBlackListInfo> capture() {
			...
			for (RxNode nodeDl : lstDl) {
				...
				try {
					...
					String company_name = StringUtils.trim(nodeDl.getNodeByXpath("./dd[1]").getText());
					String organization_id = StringUtils.trim(nodeDl.getNodeByXpath("./dd[2]").getText());
					String area = StringUtils.trim(nodeDl.getNodeByXpath("./dd[3]").getText());
					CompanyBlackListInfo entity = new CompanyBlackListInfo();
					entity.setCompany_name(company_name);
					entity.setOrganization_id(organization_id);
					entity.setArea(area);
					entity.setBlacklist_type(blacklist_name);
					lstEntity.add(entity); 
